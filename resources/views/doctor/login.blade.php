...
<body>
    ...
    <div class="login-container d-md-flex">
        <div class="login-image loginDoc"></div>
        <div class="authentication-container">
            <h1 class="title">Sign In Doctor</h1>
            <p class="mb-4">Please enter your credentials.</p>
        <!-- Session Status -->
        <x-auth-session-status class="mb-4" :status="session('status')" />
            <form method="POST" action="{{ route('loginDoc') }}" class="pt-4">
            @csrf
                            <!-- Email Address -->
                ...
                <div class="mb-2">Want to Login As User? <a href="{{route('loginDoc')}}">Log In</a></div>
                <div class="mb-2">Return without login <a href="{{route('main')}}">Home</a></div>
            </form>
        </div>
    </div>
